...
switch Mode
    ...
    case 2
        ...
        isFigure=0;
        threshold=30;
        storeAll=0; % we will not store all the temperaure information anymore
        interval=1;
        filename=strcat('c_L_',num2str(L),'_num_',num2str(num_total),...
            '_mid_',num2str(num_mid),'_side_',num2str((num_total-num_mid)/2),'_pad_',num2str(interval),'_.mat');
        if exist(filename)==0
    ...
    case 4
        ...
        filename=strcat('cRan_L_',num2str(L),'_num_',num2str(num_total),...
            '_mid_',num2str(num_mid),'_side_',num2str((num_total-num_mid)/2)...
            ,'_pad_',num2str(interval),'_.mat');
    
        if exist(filename)==0
            c=nchoosek(1:(L-2*interval)*(L-interval),num_total);% different for Random.
            rang=size(c,1);
            total_list=randperm(rang,rang);
            save(filename,'c','rang','total_list');
        else
